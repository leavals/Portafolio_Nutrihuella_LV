...
async function request<T = any>(path: string, opts: Opts = {}) {
  ...
  try {
    const res = await fetch(url, {
      ...
      // Usamos omit: todo va por Authorization; no necesitamos cookies
      credentials: 'omit',
    });

    // leemos SIEMPRE el cuerpo como texto para poder loguearlo
    ...
    if (!res.ok) {
      ...
      // 🔥 Log detallado para ver el verdadero problema
    ...
    // si no hay cuerpo devolvemos null
    if (!text) return null as any;

    // devolvemos JSON si es JSON; si no, el texto tal cual
    const ct = res.headers.get('content-type') || '';
    return (ct.includes('application/json') ? json : (text as any)) as T;
  } catch (e: any) {
    // Los fallos de red/CORS en fetch lanzan TypeError: Failed to fetch
    ...
    if (isNetwork) {
      ...
      throw new Error(
        'No se pudo conectar con el backend. Verifica que el servidor en :4000 esté encendido y CORS permita Authorization.'
      );
    }
    throw e;
  }
